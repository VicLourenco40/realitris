...
public class GridsManager : MonoBehaviour
{
    ...
    void Start() {
        ...
        game.pieceCreated.AddListener(UpdateNextGrid);
        game.pieceHeld.AddListener(UpdateHoldGrid);
    ...
    private void InitializeGameGrid() {
        for (int row = 0; row < game.GridRows; row++) {
            for (int column = 0; column < game.GridColumns; column++) {
                ...
                block.transform.parent = GameObject.Find("Grid Blocks").transform;
    ...
    public void UpdateNextGrid() {
        if (game != null) {
            ...
            UpdateGrids("Next Blocks", nextPiece);
        }
    }

    public void UpdateHoldGrid() {
        if (game != null) {
            UpdateGrids("Hold Blocks", game.holdPiece);
        }
    }
